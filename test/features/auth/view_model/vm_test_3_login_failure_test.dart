...
void main() {
  ...
  testWidgets('3. login failure sets status to error with message', (tester) async {
    when(() => mockLogin.call(any())).thenAnswer((_) async => Left(ApiFailure(message: 'Invalid credentials')));
    ...
    await runWithContext(tester, container, (ctx) =>
      container.read(authViewModelProvider.notifier).login(
        context: ctx, email: tEmail, password: tPassword,
      ),
    );
