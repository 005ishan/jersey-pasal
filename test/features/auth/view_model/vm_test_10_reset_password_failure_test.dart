...
void main() {
  ...
  testWidgets('10. resetPassword failure sets status to error', (tester) async {
    when(() => mockResetPassword.call(any())).thenAnswer((_) async => Left(ApiFailure(message: 'Reset failed')));
    ...
    await runWithContext(tester, container, (ctx) =>
      container.read(authViewModelProvider.notifier).resetPassword(
        context: ctx, email: tEmail, newPassword: 'newPass123',
      ),
    );
